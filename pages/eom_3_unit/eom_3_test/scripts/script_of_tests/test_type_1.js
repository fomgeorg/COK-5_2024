    ...
    if (window.dataLoaded) {
        ...
        function createTest(index) {
            ...
            if (imageInfo && test.find(item => item.type === 1) || test.find(item => item.type === 2)) {
                ...
                if(imageInfo.unique_class){
                    let contentDiv = document.querySelector('#dynamic-content')
                    imageDiv.classList.add(`unique_img_${index}`);
                    contentDiv.classList.add(`unique_div_${index}`);
                }
                let img;
                if(imageInfo){
