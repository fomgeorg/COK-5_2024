...
if (imageObj && imageObj.image) {
    ...
    if(imageObj.unique_class){
        mainWrapper.classList.add(`unique_div_${currentPageIndex}`);
        imgElement.classList.add(`unique_img_${currentPageIndex}`);
    }
...
function swapItems(fromIndex, toIndex) {
    ...
    storeItems = [];
    for (i of listItems) {
        storeItems.push(i.children[1].innerHTML);
    }
    localStorage.setItem('data_for_list', JSON.stringify(storeItems));
}
function checkAnwser() {
    ...
    for (let i = 0; i < listItems[0].children.length; i++) {
        const itemText = listItems[0].children[i].getElementsByTagName('div')[0].innerHTML;
